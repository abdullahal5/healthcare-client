...
const HCFileUploader = forwardRef(function HCFileUploader(
  ...
  const handleFileChange = useCallback(
    (selectedFiles: File[]) => {
      ...
      // Clear previews and files if default was set
      setPreviews({});
      setFiles([]);
    ...
    [files, maxFiles, maxSize, multiple, onFileChange]
  );
  ...
  useEffect(() => {
    if (!hasSetDefault && defaultValue && typeof defaultValue === "string") {
      const fileName = "Current profile photo";
      const fakeFile = new File([""], fileName, { type: "image/*" });

      setFiles([fakeFile]);
      setPreviews({ [fileName]: defaultValue });

      if (onFileChange) {
        onFileChange(multiple ? [fakeFile] : fakeFile);
      }

      setHasSetDefault(true);
    }
  }, [defaultValue, onFileChange, multiple, hasSetDefault]);
              ...
              {files?.length > 0 && (
                <div className="w-full space-y-4">
                  ...
                  {files?.map((file, index) => (
                    <Card
                      key={index}
                      className="w-full mx-auto transition-all animate-in fade-in"
                    >
                      ...
                      <CardContent>
                        ...
                        file?.type?.startsWith("image") ? (
                          <div className="mt-2 rounded-lg overflow-hidden border">
                            <Image
                              src={previews[file?.name] ?? defaultValue}
